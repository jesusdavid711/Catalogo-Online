...
public class EventService {
    ...
    public EventDTO updateEvent(Long id, EventDTO eventDTO) {
        ...
        if (eventDTO.getVenueId() != null &&
                !eventDTO.getVenueId()
                        .equals(existingEntity.getVenue() != null ? existingEntity.getVenue().getId() : null)) {
            venue = venueRepository.findById(eventDTO.getVenueId())
                    .orElseThrow(() -> new NotFoundException(
                            "Venue no encontrado con ID: " + eventDTO.getVenueId()));
        }
        ...
        // Categories are now handled as Set<CategoryEntity> and managed in the adapter
        // layer
        // existingEntity.setCategories(...) will be handled separately
    ...
    public Page<EventDTO> getEventsWithFilters(String city, String category,
        ...
        org.springframework.data.jpa.domain.Specification<EventEntity> spec = com.example.CatalogoOnline.repository.EventSpecifications
                .withDynamicFilters(city, category, startDate);

        // Use the optimized findAll with EntityGraph to avoid N+1
